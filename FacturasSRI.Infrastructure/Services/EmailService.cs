    ...
    public class EmailService : IEmailService
    {
        ...
        private string BuildEmailTemplate(string title, string bodyContent, string buttonUrl, string buttonText)
        {
            ...
            return html.ToString();
                ...
                public async Task SendCustomerTemporaryPasswordEmailAsync(string toEmail, string customerName, string temporaryPassword)

                {

                    var subject = $"¡Bienvenido a {_fromName}! Tu cuenta ha sido creada.";

                    var baseUrl = _configuration["App:BaseUrl"] ?? "http://localhost:5000";

                    string loginUrl = $"{baseUrl}/login"; // Assuming a customer-specific login page
                    ...
                    var htmlContent = BuildEmailTemplate($"¡Bienvenido, {customerName}!",

                        "<p>Tu cuenta ha sido creada exitosamente por nuestro equipo. A continuación encontrarás tus credenciales de acceso:</p>" +
                        ...
                        "<p>Por tu seguridad, te recomendamos cambiar esta contraseña la primera vez que inicies sesión en el sistema.</p>",

                        loginUrl, "Iniciar Sesión Cliente");
